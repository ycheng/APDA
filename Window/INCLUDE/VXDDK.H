...
#define	TOUCHPADWIDTH		0xfff
#define	TOUCHPADHEIGHT		0xfff

#define	tPadCaliWidth		(Coord)100
#define	tPadCaliHeight		(Coord)100

#define	VTPADToPDAXCoord(x)	(Coord)((x*tPadCaliWidth/tPadDiffX)-tBaseX)
#define	VTPADToPDAYCoord(y)	(Coord)((y*tPadCaliHeight/tPadDiffY)-tBaseY)

#define	APIClassName		"APITEST"
#define	CursorSize		16

#define	ScreenWidth		(Coord)440
#define	ScreenHeight		(Coord)670
...
extern void CreateWindows(char* appName, 
						  ...
						  int Width, 
						  int Height);
extern void VPeekEventsDDK(void);
extern void WResetEvent (EventType *event);

struct tagDisplay;

typedef int  (*pfnGetPixel) (struct tagDisplay* display, int x, int y);
typedef void (*pfnPutPixel) (struct tagDisplay* display, int x, int y, int color);
typedef void (*pfnUpdateScreen) (struct tagDisplay* display);

typedef struct tagRectangle {
  int uLeft, uRight, uTop, uBottom;
} WRectangle;
...
typedef struct tagDisplay {
  ...
  pfnPutPixel pfnPutPixel;
  pfnUpdateScreen	pfnUpdateScreen;
  void* pPrivate;
  void* pZBuffer;
} Display;
