...
BOOL CMapiSession::ReadFirst(char *MessageID, CMailMessage *message)
{
	//For correct operation of the T2A macro, see MFC Tech Note 59
	USES_CONVERSION;
  ULONG	nError;
  ...
  if (m_lpfnMAPIFindNext(m_hSession, 0, NULL, NULL, 0, 0, MessageID)==SUCCESS_SUCCESS) {
	  nError = m_lpfnMAPIReadMail(m_hSession, 0, MessageID, 0, 0, &mapiMessage);
  }
  ...
  if (nError == SUCCESS_SUCCESS)
  {
	...
	message->m_sFrom = mapiMessage->lpOriginator->lpszName;
	message->m_sFromAddress = mapiMessage->lpOriginator->lpszAddress;
	for ( ULONG i = 0; i < mapiMessage->nRecipCount; i++ ) {
	    MapiRecipDesc& recip = mapiMessage->lpRecips[i];
		if ( i > 0 ) {
			message->m_sTo += _T("; ");
			message->m_sToAddress += _T("; ");
		}
		message->m_sTo += recip.lpszName;
		message->m_sToAddress += recip.lpszAddress;
	}
//	message->m_tDateTime = mapiMessage->lpszDateReceived;
  }
  else
  {
    TRACE(_T("Failed to send mail message, Error:%d\n"), nError);
...
BOOL CMapiSession::ReadNext(char *MessageID, CMailMessage *message)
{
	//For correct operation of the T2A macro, see MFC Tech Note 59
	USES_CONVERSION;
  ...
  if (m_lpfnMAPIFindNext(m_hSession, 0, NULL, MessageID, 0, 0, MessageID)==SUCCESS_SUCCESS) {
	  nError = m_lpfnMAPIReadMail(m_hSession, 0, MessageID, 0, 0, &mapiMessage);
  }
